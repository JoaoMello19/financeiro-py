...
def plot_last_closings(data, ticker_name, period="1y"):
    def add_graph1(ax):
        ...
        ax.plot(data.index, data['MM_7'], label='Média Móvel 7 dias', color='green', linestyle='--')
...
def plot_covariance(data, ticker, title_sufix=None):
    ...
    title = f"Mapa de Calor - Correlação de {ticker}"
    ...
    # Caminho do arquivo
    filename = f"heatmap_{ticker.replace('.', '_')}"
    if title_sufix is not None:
        filename += f"_{title_sufix}"
    filename += ".png"
